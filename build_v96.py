# -*- coding: utf-8 -*-
"""v9.6 打包：在 v9.4.1 基础上加入

- 钉钉连接器接入 dws 工作台 CLI（官方 DingTalk Workspace CLI）：
  个人账号 OAuth 授权后即可「列出并读取会话列表/消息」（群聊+单聊），
  并能在界面/由 Agent 发消息。这正是 WorkBuddy 钉钉连接器的真实做法，
  也修正了之前 self_test 与提示词里"平台不提供会话列表"的错误结论。
- 集成页新增「钉钉工作台 CLI」区域：浏览器/设备码授权登录、查看会话列表、
  双击读消息、直接发消息。
- Agent 的 dingtalk 工具新增 conversations / messages / dws_send / dws_status 四个 action。
- 提示词 PROMPT_VERSION 17 -> 18（明确说明 dws 路线可读会话列表）。
...
- 本机同时装了 PyQt5（别的项目用的），PyInstaller 检测到多 Qt 绑定会直接 abort，
  所以 PyQt5/PySide 全排除。
- 每次重打前必须把旧 build/ 与 dist_v9/ 先 mv 走（沙箱同 turn 删 >50 文件会被拦）。
"""
import os
import sys
import subprocess
...
DIST = os.path.join(PROJ, "dist_v96")

# ---- 生成 exe 版本信息（资源管理器里能看到版本号/公司/说明）----
VERSION_FILE = os.path.join(PROJ, "build_v96_version.txt")
