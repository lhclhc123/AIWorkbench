# -*- coding: utf-8 -*-
"""v9 打包：在 v8（31 个工具）基础上加入

- 语音输入（pyaudio 录音 + 智谱 glm-asr 识别）
- 语音输出（Windows SAPI 离线 / edge-tts 在线）
- 钉钉接入（群机器人 webhook 加签 + 企业内部应用 + Stream 长连接）
- 自动更新（读 GitHub Release，无需 API token）
- 密钥保护（本地混淆 + DPAPI 封装，exe 内不含明文）
...
VERSION_FILE = os.path.join(PROJ, "build_v9_version.txt")
